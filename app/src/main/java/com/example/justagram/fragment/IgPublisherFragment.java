...
public class IgPublisherFragment extends Fragment {
    ...
    private void uploadAllImagesToServer(String serverUrl, List<Uri> uris, UploadAllCallback cb) {
        ...
        if (uris.isEmpty()) {
            runOnMain(() -> cb.onDone(null, "No URIs"));
            return;
        }
        ...
        for (Uri u : uris) {
            try {
                ...
                if (in == null) {
                    runOnMain(() -> cb.onDone(null, "Cannot open file"));
                    return;
                }
    ...
    private void uploadVideoToServer(String serverUrl, Uri videoUri, UploadVideoCallback cb) {
        ...
        try {
            ...
            if (in == null) {
                runOnMain(() -> cb.onDone(null, "Cannot open video file"));
                return;
            }
            byte[] bytes = new byte[in.available()];
            in.read(bytes);
    ...
    private void showMsg(String s) {
        runOnMain(() -> {
            try {
                Utility.showMessageBox(s, getContext());
            } catch (Exception e) {
    ...
    private class PreviewAdapter extends RecyclerView.Adapter<PreviewVH> {
        ...
        public void onBindViewHolder(@NonNull PreviewVH holder, int position) {
            holder.img.setImageURI(selectedUris.get(position));
        }
